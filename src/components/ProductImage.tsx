...
export default function ProductImage({ imageUrl, category, alt, className = "" }: ProductImageProps) {
  ...
  const hasImage = !!imageUrl && !isError;

  const fallbackContent = (style?: React.CSSProperties) => (
    <div 
      className={`w-full h-full flex items-center justify-center bg-stone-900/60 relative overflow-hidden group ${className}`}
      style={style}
    >
      {/* Background soft glow / shimmer skeleton */}
      <div className="absolute inset-0 bg-gradient-to-r from-stone-900 via-stone-850 to-stone-900 animate-pulse pointer-events-none" />
      <div className="absolute inset-0 bg-radial from-amber-500/10 via-transparent to-transparent opacity-60" />
      
      {category === "honey" && (
        <svg className="w-20 h-20 text-amber-500 drop-shadow-md transition-transform duration-300 group-hover:scale-110 relative z-10" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
      ...
      {category === "amlou" && (
        <svg className="w-20 h-20 text-amber-800 drop-shadow-md transition-transform duration-300 group-hover:scale-110 relative z-10" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
      ...
      {category === "argan" && (
        <svg className="w-20 h-20 text-yellow-600 drop-shadow-md transition-transform duration-300 group-hover:scale-110 relative z-10" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
      ...
      <span className="absolute bottom-2 text-[10px] font-mono tracking-widest text-amber-500/80 bg-stone-950/85 px-2.5 py-1 rounded-full uppercase opacity-90 border border-stone-800/40 z-10">
        {category}
      </span>
    </div>
  );
  ...
  if (hasImage) {
    return (
      <div className={`relative w-full h-full overflow-hidden bg-stone-900 ${className}`}>
        {/* Placeholder Skeleton shown until image loads */}
